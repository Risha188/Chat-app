...
const Sidebar = () => {
    ...
    return (
        <div className={`bg-[#8581B2]/10 h-full p-5 overflow-y-scroll rounded-r-xl text-white ${selectedUser ? 'max-md:hidden' : ''}`}>
            <div className='pb-5'>
            ...
            <div className='flex flex-col'>
                { filteredUsers.map((user,index) => (
                    <div onClick={() => {
                        ...
                        {unseenMessages[user._id] > 0 && <p className='absolute top-4 right-4 text-xs rounded-full flex justify-center items-center h-5 w-5 bg-violet-500/50'>{unseenMessages[user._id]}</p>}
                    </div>
                ))}
            </div>
        </div>
